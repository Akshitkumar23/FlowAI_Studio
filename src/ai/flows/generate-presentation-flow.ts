...
const expandContentPrompt = ai.definePrompt({
  ...
  Given the following slide title and content, expand on the bullet points to make them more detailed and descriptive, while maintaining the same number of bullet points.
...
const shortenContentPrompt = ai.definePrompt({
  ...
  output: {schema: SlideContentOutputSchema},
  prompt: `You are an expert at refining presentation content.
  Given the following slide title and content, shorten the bullet points to make them more concise and punchy, while maintaining the same number of bullet points.
