...
const ResumePreview = ({ data, photoPreview, leftColumnColor, headingColor }: { data: Partial<ResumeFormValues>, photoPreview: string | null, leftColumnColor: string, headingColor: string }) => {
    ...
    return (
        <div className="w-[8.5in] h-[11.95in] bg-white text-black p-8 shadow-lg font-sans text-base origin-top-left">
            <div className="grid grid-cols-3 gap-8">
                ...
                <div className="col-span-1 p-6 rounded-md space-y-8" style={leftColumnStyle}>
                    ...
                    {data.languages && data.languages.length > 0 && (
                        <div className="space-y-4">
                            <h3 className="text-sm font-bold uppercase tracking-widest border-b-2 pb_2" style={{ borderColor: headingColor, color: headingColor }}>Languages</h3>
                            <ul className="space-y-1 text-xs list-disc list-inside">
                    ...
                    {data.certificates && data.certificates.length > 0 && (
                        <div className="space-y-4">
                            <h3 className="text-sm font-bold uppercase tracking-widest border-b-2 pb_2" style={{ borderColor: headingColor, color: headingColor }}>Certificates</h3>
                            <ul className="space-y-1 text-xs list-disc list-inside">
  ...
  useEffect(() => {
    const calculateScale = () => {
      if (previewCardRef.current) {
        ...
        const padding = 32; // Corresponds to p-4 in Tailwind
  ...
  return (
    <>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
              ...
              <div className="space-y-6">
                  ...
                  <form className="space-y-6">
                      <Card className="glass">
                          <CardHeader><CardTitle className="flex items-center gap-2"><Palette /> Theme & Colors</CardTitle></CardHeader>
                          <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-4">
                              <div className="space-y-2">
                                  ...
                                  <div className="flex items-center gap-2">
                                      ...
                                      <Input type="text" value={leftColumnColor} onChange={(e) => setLeftColumnColor(e.target.value)} />
                                  </div>
                              </div>
                               <div className="space-y-2">
                          ...
                          <CardContent className="space-y-4">
                             <div className="flex items-center gap-4">
                                 <div className="w-24 h-24 rounded-full bg-muted flex items-center justify-center border">
                                    {photoPreview ? <Image src={photoPreview} alt="preview" width={96} height={96} className="rounded-full object-cover"/> : <User className="w-10 h-10 text-muted-foreground"/>}
                                 </div>
                      ...
                      <Card className="glass">
                          ...
                          <CardContent className="space-y-4">
                              {workExperience.map((field, index) => (
                                  <div key={field.id} className="p-4 border rounded-md relative space-y-2 bg-background/20">
                              ...
                              <Button type="button" variant="outline" onClick={() => appendWork({ jobTitle: "", company: "", startDate: "", endDate: "", description: "" })}><PlusCircle className="mr-2"/> Add Experience</Button>
                          </CardContent>
                      </Card>
                      ...
                      <Card className="glass">
                          ...
                          <CardContent className="space-y-4">
                              ...
                              <Button type="button" variant="outline" onClick={() => appendEducation({ degree: "", institution: "", startDate: "", endDate: "", percentage: "" })}><PlusCircle className="mr-2"/> Add Education</Button>
                          </CardContent>
                      </Card>

                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                          <Card className="glass">
                              <CardHeader><CardTitle className="flex items-center gap-2 text-base"><Star /> Skills</CardTitle></CardHeader>
                              <CardContent className="space-y-4">
                                  {skills.map((field, index) => (
                                      <div key={field.id} className="flex items-center gap-2">
                                          <FormField name={`skills.${index}.value`} render={({ field }) => (
                                              <FormItem className="flex-grow"><FormControl><Input {...field} placeholder="e.g. Project Management"/></FormControl></FormItem>
                                          )}/>
                                          <Button type="button" variant="ghost" size="icon" onClick={() => removeSkill(index)}><Trash2 className="h-4 w-4 text-red-500"/></Button>
                                      </div>
                                  ))}
                          ...
                          <Card className="glass">
                              <CardHeader><CardTitle className="flex items-center gap-2 text-base"><Globe /> Languages</CardTitle></CardHeader>
                              <CardContent className="space-y-4">
                                  {languages.map((field, index) => (
                                      <div key={field.id} className="flex items-center gap-2">
                                          <FormField name={`languages.${index}.value`} render={({ field }) => (
                                              <FormItem className="flex-grow"><FormControl><Input {...field} placeholder="e.g. English"/></FormControl><FormMessage /></FormItem>
                                          )}/>
                                          <Button type="button" variant="ghost" size="icon" onClick={() => removeLanguage(index)}><Trash2 className="h-4 w-4 text-red-500"/></Button>
                                      </div>
                                  ))}
                                  <Button type="button" variant="outline" size="sm" onClick={() => appendLanguage({value: ""})}><PlusCircle className="mr-2 h-4 w-4"/> Add Language</Button>
                              </CardContent>
                          </Card>
                      </div>

                      <Card className="glass">
                          <CardHeader><CardTitle className="flex items-center gap-2 text-base"><Award /> Certificates</CardTitle></CardHeader>
                          <CardContent className="space-y-4">
                              {certificates.map((field, index) => (
                                  <div key={field.id} className="flex items-center gap-2">
                                      <FormField name={`certificates.${index}.value`} render={({ field }) => (
                                          <FormItem className="flex-grow"><FormControl><Input {...field} placeholder="e.g. Certified React Developer"/></FormControl><FormMessage /></FormItem>
                                      )}/>
                                      <Button type="button" variant="ghost" size="icon" onClick={() => removeCertificate(index)}><Trash2 className="h-4 w-4 text-red-500"/></Button>
                                  </div>
                              ))}
                              <Button type="button" variant="outline" size="sm" onClick={() => appendCertificate({value: ""})}><PlusCircle className="mr-2 h-4 w-4"/> Add Certificate</Button>
                          </CardContent>
                      </Card>

                      <div className="space-y-4 pt-4">
                        <Button type="button" size="lg" onClick={handleBuildWithAI} disabled={isBuildingWithAI} className="w-full bg-accent hover:bg-accent/90 text-accent-foreground">
                        ...
                        <Button type="button" size="lg" onClick={() => setIsPreviewOpen(true)} className="w-full">
              ...
              <div className="lg:sticky lg:top-8">
                  <Card className="glass">
                      ...
                      <CardContent ref={previewCardRef} className="p-2 sm:p-4 flex justify-center items-start">
      ...
      <Dialog open={isPreviewOpen} onOpenChange={setIsPreviewOpen}>
        <DialogContent className="max-w-4xl w-full h-[90vh] flex flex-col p-0 bg-background/80 backdrop-blur-md">
            ...
            <DialogFooter className="p-4 border-t sm:justify-between">
                <Button variant="secondary" onClick={handleBuildWithAI} disabled={isBuildingWithAI}>
                    {isBuildingWithAI ? <Loader2 className="mr-2 h-4 w-4 animate-spin"/> : <Wand2 className="mr-2 h-4 w-4" />}
